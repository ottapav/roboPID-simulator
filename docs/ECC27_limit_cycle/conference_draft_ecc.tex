...
Table~\ref{tab:duty} tests these predictions on the whole battery and every
plant confirms them exactly, the shares matching
$(\tfrac18,\tfrac12,\tfrac14,\tfrac18)$ to three decimal places in all four
cases. Three plants close a period-$8$ orbit; P3 closes a period-$16$ orbit
with counts $(2,8,4,2)$. Corollary~\ref{cor:period} predicts not merely
period $8$ but any multiple of eight with counts $n_e(1,4,2,1)$, and P3 is the
only member of the battery that tests the multiple.

\begin{observation}[Ordering is Plant-Dependent]
\label{obs:ruler}
The counts are universal but the order of the moves is not: each plant closes a
different sequence. P2 gives the ruler ordering $d_0 d_1 d_0 d_2 d_0 d_1 d_0 e$,
a cube of side $h$ (Fig.~\ref{fig:cycle}); P4 gives
$d_0 e\, d_0 d_2 d_0 d_1 d_0 d_1$ and P1 gives
$d_1 d_1 e\, d_0 d_2 d_0 d_0 d_0$, flatter polygons of the same period and
counts.
\end{observation}

Corollaries~\ref{cor:triple} and~\ref{cor:period} fix the frequencies and the
length; which ordering a plant selects is a property of its switching surfaces
and remains a measurement. For practice this does not matter, because the
counts alone bound the excursion whatever the ordering turns out to be.
...
Table~\ref{tab:duty} reports the measured half-spans: every plant lies inside
the bound, and P1 attains it in the proportional and derivative coordinates.

\begin{figure}[t]
\centering
\includegraphics[width=0.82\linewidth]{fig_limit_cycle}
\caption{One period of the terminal orbit of P2, at $\beta = 0.10$ under the
regularized index. Its measured ordering is the ruler sequence
$d_0 d_1 d_0 d_2 d_0 d_1 d_0 e$, so the eight points are the corners of a cube
of side $h$ visited in binary counting order: $\log K_i$ is the least
significant bit, $\log K_p$ and $\log K_d$ the higher bits, and $e$ the carry
that closes the period. The cross marks the centre. This is the tightest of the
four orbits of Table~\ref{tab:duty}.}
\label{fig:cycle}
\end{figure}
...
\small
\centering
\caption{Terminal orbits of the whole battery, from $2000$-iteration runs at
$\beta = 0.10$ from unit gains with the regularized index \eqref{eq:neps}.
Every plant closes an \emph{exactly} periodic orbit, no block departing from
the pattern over the second half of the run, with move shares matching
Proposition~\ref{prop:balance} to three decimals and counts $n_e(1,4,2,1)$ as
Corollary~\ref{cor:period} requires. Half-spans are in units of $h$, in
$(\log K_i, \log K_p, \log K_d)$. Under the published hard count the same runs
miss the shares by up to $0.04$ and close no orbit at all.}
\label{tab:duty}
\begin{tabular}{lccccc}
\toprule
Plant & period & $n_e$ & counts & half-span & bound \\
\midrule
P1 & $8$  & $1$ & $(1,4,2,1)$ & $(1.5,1,\tfrac12)$ & $(2,1,\tfrac12)$ \\
P2 & $8$  & $1$ & $(1,4,2,1)$ & $(\tfrac12,\tfrac12,\tfrac12)$ & $(2,1,\tfrac12)$ \\
P3 & $16$ & $2$ & $(2,8,4,2)$ & $(3,1.5,1)$ & $(4,2,1)$ \\
P4 & $8$  & $1$ & $(1,4,2,1)$ & $(\tfrac12,1,\tfrac12)$ & $(2,1,\tfrac12)$ \\
\bottomrule
\end{tabular}
\end{table}

\paragraph{Boundary Case} If $\theta^*$ lies outside the gain box the
trajectory settles instead where, for each band, either the index sits on its
threshold or the gain sits on a bound: a complementarity point. The companion
paper's P2 run ends this way with $K_d$ on the floor of $0.01$, whereas from
unit gains under the regularized index the same plant reaches an interior orbit
(Table~\ref{tab:duty}) --- so this is a property of the starting gains, not of
the plant.
...
The bound contains no plant parameter and is deliberately asymmetric: the
integral gain is moved by all four moves and the derivative gain by two, so
$K_i$ is the loosest coordinate and $K_d$ the tightest. At the companion
paper's default $\beta = 0.1$ the guarantees are $23.5\,\%$ on $K_i$,
$11.1\,\%$ on $K_p$ and $5.4\,\%$ on $K_d$; holding all three within
$5\,\%$ requires $\beta \le 0.024$. The multiplicity $n_e$ is not assumed but
read from the run, being the number of expand moves per period: it is $1$ on
three plants of the battery and $2$ on P3 (Table~\ref{tab:duty}), and the
factors compound with it.
...
orbit whose length is a multiple of eight with counts $n_e(1,4,2,1)$, and
those counts alone bound how far each gain strays from its centre. All four
plants of the battery close such an orbit exactly, three with $n_e=1$ and one
with $n_e=2$. If the
